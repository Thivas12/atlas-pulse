...
| PostgreSQL/Valkey → API | Public events, graph, search, freshness, evidence packs, blocked run manifests | Read-only HTTP surface, typed response contracts, strict query limits and cursors, no arbitrary server-side URL fetch, atomic pseudonymous per-client general and expensive-route budgets | There is no user authentication or upstream WAF; a distributed source-address attack can exceed per-client controls |
...
| Public evidence → local human review | Rank-blind CSV and terminal-rendered source text | Spreadsheet-formula neutralization, terminal-control stripping, exact protected-field validation, atomic per-decision replacement, and Git-ignored artifacts | A compromised workstation can alter evidence or observe reviews; distinct names do not prove that people worked independently |
| Public internet → workstation Funnel edge | TLS-terminated HTTP plus PROXY protocol v2 source metadata | Tailscale-managed public TLS; no router forwarding; loopback-only Caddy port; required PROXY header on every edge connection; synchronized HSTS and browser headers | Tailscale, the local client, workstation, Docker daemon, power, and home connection remain trusted; Funnel is beta, bandwidth-limited, and is not a WAF |
...
| T2 | Oversized downloads, decompression bombs, or expensive queries exhaust a small host | HTTP timeouts/retry caps; GDELT compressed, uncompressed, row, and event caps; bounded API limits; container CPU/memory limits; proxy-normalized client identity; atomic Valkey-backed general and expensive-route request budgets with explicit `429` and fail-closed quota errors | Static/TLS work occurs before the application limiter, source-address distribution bypasses per-client quotas, and the public edge has no WAF. Tune quotas from observed load before advertising availability guarantees |
...
| T10 | A compromised internal service moves laterally through shared infrastructure | Diagnostic host ports bind to loopback; only the reviewed public ingress chain reaches the internet; exact two-member internal networks isolate every client/data-service link and both proxy hops; each host-access or purpose-specific egress bridge contains one service; injected database, Valkey, FIRMS, and approval settings are scoped to required processes; each public overlay enforces a 32-character minimum URL-safe database password format | PostgreSQL and Valkey remain central trust points, four processes share one application database role, non-internal host bridges may permit outbound traffic, host/Docker administrators bypass container isolation, password randomness remains an operator responsibility, and the local-development profile intentionally retains a known password for loopback use |
...
| T13 | A public tunnel bypasses the edge, loses the real client identity, or makes workstation availability look like an SLA | Funnel targets only `127.0.0.1:8443`; the rendered-model gate enforces that binding; TLS-terminated TCP sends PROXY v2; Caddy requires the header, applies HSTS, and forwards through the existing strict proxy chain; the runbook states power, sleep, update, connection, beta-service, and sampling limits | A host administrator or other local process can forge PROXY metadata, many source addresses bypass per-client quotas, and the workstation or third-party tunnel can fail between samples |
| T14 | Crafted public text or edited review files manipulate a terminal, trigger spreadsheet formulas, reveal system rankings, or corrupt relevance gold | Rank-, score-, mode-, and reviewer-blind editable sheets; control-character stripping; formula neutralization; byte-exact evidence validation; deterministic A/B swapping; atomic resumable writes; distinct reviewer/adjudicator identities | Host compromise bypasses local controls, reviewer independence remains procedural, and human judgments can still be mistaken or biased |
...
| Public contracts | API tests, browser schema tests, UI component tests, a deterministic Chromium smoke test of production assets under the enforced header policy, atomic quota tests, and container edge/`429` smoke tests |
...
| Runtime isolation | Machine-readable trust graphs for base, VM, and workstation profiles; semantic rendered-model validation; credential-owner checks; public secret gates; and pinned-Caddy validation of the required workstation PROXY listener |
